...
const MathData = {
    ...
    _frameComposing(level) {
        ...
        if (target === 5) {
            ...
            // Ten-frame: two rows of 5 (use Array.from for proper emoji splitting)
            const allChars = Array.from(filled + empty);
            const row1 = allChars.slice(0, 5).join('');
            const row2 = allChars.slice(5).join('');
            visual = `[ ${row1} ]\n[ ${row2} ]`;
        }
    ...
    _size2nd(level) {
        ...
        if (templateType === 0) {
            ...
            if (a === b) {
                const sameAnswers = this._shuffle(['Same length', `${a + 1} ${unit}`, `${a - 1} ${unit}`]);
                return {
                    question: `Compare: ${a} ${unit} and ${b} ${unit}`,
                    questionSpeak: `Compare ${a} ${unit} and ${b} ${unit}`,
                    answers: sameAnswers,
                    correctIndex: sameAnswers.indexOf('Same length'), topic: 'size',
                    subtype: 'compare-measure',
