...
const GameRenderer = {
    ...
    _drawFinishLine() {
        ...
        for (let row = 0; row < 11; row++) {
            for (let col = 0; col < 3; col++) {
                ...
                ctx.fillRect(flagX + col * tileSize, roadTop + row * tileSize, tileSize, tileSize);
            }
        }

        const progress = 1 - (this.finishTimer / 90);
    ...
    _drawBackground() {
        ...
        skyGrad.addColorStop(0.5, theme.bg);
        skyGrad.addColorStop(0.85, this._lightenRoad(theme.bg, 15));
    ...
    _drawSceneryItem(ctx, type, x, baseY, side, scale, seed) {
        ...
        const hasShadow = type !== 'post' && type !== 'tumbleweed' && type !== 'barrier';
        if (hasShadow) {
            ctx.globalAlpha = 0.1;
            ctx.fillStyle = '#000';
            ctx.beginPath();
            const shadowW = (type.includes('building') || type === 'house') ? 14 * s : 10 * s;
            const shadowH = 3.5 * s;
        ...
        const isTree = type === 'tree_deciduous' || type === 'pine_tree' || type === 'palm_tree';
    ...
    _drawObstacles() {
        ...
        this.obstacles.forEach(obs => {
            ...
            if (obs.hit) ctx.globalAlpha = 0.3;
