...
const Progress = {
    ...
    // XP thresholds for each level (extended to 40 for 50-car roster)
    LEVEL_THRESHOLDS: [
    ...
    RANK_BOUNDARIES: [1, 4, 7, 10, 13, 16, 21, 26, 31, 36], // levels where rank changes
    ...
    defaults() {
        return {
            ...
            playerLevel: 1,
            currentTrack: 0,
            tracksUnlocked: [true, false, false, false, false, false, false, false],
            carColor: 'red',
            carsUnlocked: ['red'],
    ...
    VALID_CAR_TYPES: ['c1','c2','c3','c4','c5','c6','c7','c8',
                      ...
                      'batmobile','policecar','ambulance','towtruck','icecreamtruck','gokart','limo','tank',
                      'zamboni','tractor','grandam','focus','bronco2023','ferrari'],
    ...
    _validate(data) {
        ...
        data.totalXP = this._clampInt(data.totalXP, 0, 100000);
        data.playerLevel = this._clampInt(data.playerLevel, 1, 40);
        ...
        // Boolean arrays
        if (!Array.isArray(data.tracksUnlocked)) data.tracksUnlocked = [true, false, false, false, false, false, false, false];
        while (data.tracksUnlocked.length < 8) data.tracksUnlocked.push(false);
        data.tracksUnlocked.length = 8;
        ...
        // Grade level — V20: added '2nd'
        if (!['prek', 'k', '1st', '2nd'].includes(data.gradeLevel)) data.gradeLevel = 'prek';
    ...
    _getRankForLevel(level) {
        return this.RANK_NAMES[level] || 'Legend';
    },
    ...
    getRank() {
        return this.RANK_NAMES[this.data.playerLevel] || 'Legend';
    },
    ...
    // ---- ANSWERS & ACCURACY ----
    recordAnswer(subject, topic, correct) {
        // V18: Validate inputs
