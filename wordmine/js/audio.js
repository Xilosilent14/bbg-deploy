...
const Audio = (() => {
    ...
    function blockBreak() {
        // Layered impact: MP3 thock (if cached) + low crumble tail + noise burst
        // ensures Fire tablet Silk has audible feedback even without MP3 decode.
        const hadMP3 = _playMP3('block-break', 0.5);
        // Always layer a short low-end crumble tail so impact reads as physical
        try {
            const c = getCtx();
            if (c && soundOn) {
                ...
                g1.gain.setValueAtTime(hadMP3 ? 0.04 : 0.1, now);
                g1.gain.exponentialRampToValueAtTime(0.001, now + 0.15);
                osc1.connect(g1); g1.connect(c.destination);
                osc1.start(now); osc1.stop(now + 0.16);

                // Crumble tail (filtered noise via fast pitch warble) — only if no MP3
                if (!hadMP3) {
                    const osc2 = c.createOscillator();
                    const g2 = c.createGain();
                    osc2.type = 'sawtooth';
                    osc2.frequency.setValueAtTime(80, now + 0.06);
        ...
        setTimeout(() => playTone(150, 0.08, 'square', 0.06), 60);
    }
    ...
    // Sub-100ms positive chime — schedule everything on the AudioContext clock
    // so it fires synchronously with the tap rather than waiting on setTimeout
    // or MP3 decode. Used immediately on correct-answer before any transition.
    function fastConfirm() {
        ...
        notes.forEach(n => {
            try {
                ...
                gain.gain.exponentialRampToValueAtTime(0.18, now + n.t + 0.008);
