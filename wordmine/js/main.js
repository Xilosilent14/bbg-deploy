...
const Main = (() => {
    ...
    // Spoken encouragement for wrong answers — rotates short positive phrases
    const ENCOURAGE_SPEECH = [
        "Good try!",
        "Almost!",
        "Keep going!",
        "Nice effort!",
    ...
    let encourageSpeechIdx = 0;
    function speakEncouragement() {
        try {
            if (!Audio.voiceOn) return;
            const msg = ENCOURAGE_SPEECH[encourageSpeechIdx % ENCOURAGE_SPEECH.length];
            encourageSpeechIdx++;
            Audio.speak(msg, 'gentle');
        } catch (_) {}
    }

    // Sub-100ms tap confirmation — chime + flash on the clicked button
    // Called from each mode's handleAnswer BEFORE any setTimeout/animation
    // so the child gets feedback within ~16ms of the tap.
    function fastConfirm(clickedBtn) {
        try { Audio.fastConfirm(); } catch (_) {}
        if (clickedBtn && clickedBtn.classList) {
            clickedBtn.classList.add('correct-flash');
            // Strip the animation class after it finishes so re-renders work.
            setTimeout(() => {
                try { clickedBtn.classList.remove('correct-flash'); } catch (_) {}
            }, 260);
        }
        try {
            if (typeof Adaptive !== 'undefined') Adaptive.recordResult(true);
        } catch (_) {}
    }

    function recordWrong() {
        try {
            if (typeof Adaptive !== 'undefined') Adaptive.recordResult(false);
        } catch (_) {}
        speakEncouragement();
    }
    ...
    return { init, showScreen, startGame, showToast, showUnlockReveal, showMilestone, populateResultsExtras, getEncourageMsg, celebrateStreak, triggerEffects, setQuestionHint, maybeTreasureBonus, fastConfirm, recordWrong, speakEncouragement };
})();
...
// Wire title logo img fallback (onerror moved from HTML attr to satisfy CSP)
(function() {
    var img = document.getElementById('title-logo-img');
    if (img) {
