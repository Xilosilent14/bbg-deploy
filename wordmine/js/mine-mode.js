...
const MineMode = (() => {
    ...
    function start(selectedBiome) {
        ...
        active = true;
        try { if (typeof Analytics !== 'undefined') Analytics.event('mode_start', { mode: 'mine', biome: biome }); } catch (_) {}
    ...
    function breakBlock(block) {
        ...
        if (Math.random() < 0.15) {
            setTimeout(() => {
                ...
                try { if (typeof Analytics !== 'undefined') Analytics.event('rare_drop', { mode: 'mine', kind: 'emerald', gems: 5 }); } catch (_) {}
            }, 400);
        }

        // Ultra-rare diamond cluster jackpot (~2%) — variable-reward surprise
        if (Math.random() < 0.02) {
            setTimeout(() => {
                for (let i = 0; i < 5; i++) {
                    drops.push({
                ...
                itemsCollected.diamond_cluster = (itemsCollected.diamond_cluster || 0) + 1;
                gemsEarned += 15;
                floatingTexts.push({
                    x: bx, y: by - 35,
                    text: '★ JACKPOT! +15',
                ...
                try { Audio.superCombo && Audio.superCombo(); } catch (_) {}
                try { if (typeof Analytics !== 'undefined') Analytics.event('rare_drop', { mode: 'mine', kind: 'jackpot', gems: 15 }); } catch (_) {}
            }, 500);
        }
    }
    ...
    function endGame() {
        active = false;
        try {
            if (typeof Analytics !== 'undefined') Analytics.event('mode_end', {
                mode: 'mine', biome: biome, correct: correct, total: total,
                accuracy: total > 0 ? Math.round((correct / total) * 100) : 0,
                xp: xpEarned, gems: gemsEarned, best_streak: bestStreak
            });
        } catch (_) {}
        World.stopLoop();
        document.getElementById('question-overlay').classList.remove('active');
