...
const SpeedRound = (() => {
    ...
    function start() {
        active = true;
        try { if (typeof Analytics !== 'undefined') Analytics.event('mode_start', { mode: 'speed_round' }); } catch (_) {}
    ...
    function showQuestion(q) {
        ...
        // Always narrate the prompt+word at the start of a Speed Round
        // question — helps young decoders even when the word is shown.
        setTimeout(() => {
            if (q.speak) Audio.speakWord(q.word);
            else if (q.word) Audio.speak(q.word, 'word');
        }, 200);
    }
