...
const SurvivalMode = (() => {
    ...
    function start() {
        active = true;
        try { if (typeof Analytics !== 'undefined') Analytics.event('mode_start', { mode: 'survival' }); } catch (_) {}
        correct = 0; total = 0;
    ...
    function nextQuestion() {
        ...
        if (mobHealth <= 0) {
            ...
            if (Math.random() < (isBoss ? 0.25 : 0.05)) {
                ...
                try { if (typeof Analytics !== 'undefined') Analytics.event('rare_drop', { mode: 'survival', kind: 'treasure', wave: wave, gems: bonus }); } catch (_) {}
            }
